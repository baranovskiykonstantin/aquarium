 ...
 * ------------------------------------------------------------------------- */
void uart_response(uint8_t response)
{
    switch (response) {
    case OK:
        uart_puts ("OK\r\n");
                     ...
                     time_t *daily_corr)
{
    ...
    if (datetime_current->year > datetime_corr->year
        || (datetime_current->year == datetime_corr->year && datetime_current->month > datetime_corr->month)
        || (datetime_current->year == datetime_corr->year && datetime_current->month == datetime_corr->month && datetime_current->day > datetime_corr->day)
       )
    {
        // ... and if the current time bigger than time of the setup time...
        if (datetime_current->hour > datetime_corr->hour
            || (datetime_current->hour == datetime_corr->hour && datetime_current->min > datetime_corr->min)
            || (datetime_current->hour == datetime_corr->hour && datetime_current->min == datetime_corr->min && datetime_current->sec > datetime_corr->sec)
           )
        {
...
int main(void)
{
    ...
    pwm_set_value(eeprom_read_byte(EE_ADDR_LIGHT_TOP_LEVEL));
    pwm_set_risetime(eeprom_read_byte(EE_ADDR_LIGHT_RISE_TIME));
    ...
    while (1)
    {
        ...
        // Light
        if ((loops_count % 5000) == 0) {
            if (light_mode == MODE_AUTO) {
                if (((datetime_current.hour > time_on.hour)
                     || (datetime_current.hour >= time_on.hour && datetime_current.min > time_on.min)
                     || (datetime_current.hour >= time_on.hour && datetime_current.min >= time_on.min && datetime_current.sec >= time_on.sec)
                    )
                    &&
                    ((datetime_current.hour < time_off.hour)
                     || (datetime_current.hour <= time_off.hour && datetime_current.min < time_off.min)
                     || (datetime_current.hour <= time_off.hour && datetime_current.min <= time_off.min && datetime_current.sec < time_off.sec)
                    )
                   )
                    pwm_enable();
                else
        ...
        while ((uart_chr & 0xff00) == 0) {
            ...
            if (uart_str[uart_str_indx - 1] == '\n') {
                    ...
                    )
                {
                        ...
                        uart_putc(')');
                        uart_putc(' ');
                        uart_puti(pwm_get_value(), 0);
                        uart_putc('%');
                        uart_putc(' ');
                        uart_puti(pwm_get_risetime(), 0);
                        uart_puts("min");
                         ...
                         )
                {
                        ...
                        )
                    {
                            ...
                            )
                        {
                            ...
                            pwm_set_value(uart_str_get_int(24, 3));
                            eeprom_update_byte(EE_ADDR_LIGHT_TOP_LEVEL, pwm_get_value());

                            pwm_set_risetime(uart_str_get_int(28, 2));
                            eeprom_update_byte(EE_ADDR_LIGHT_RISE_TIME, pwm_get_risetime());
                    ...
                    {
                        pwm_set_value(uart_str_get_int(12, 3));
                        eeprom_update_byte(EE_ADDR_LIGHT_TOP_LEVEL, pwm_get_value());
                        uart_response(OK);
                    }
                    ...
                    {
                        pwm_set_risetime(uart_str_get_int(11, 2));
                        eeprom_update_byte(EE_ADDR_LIGHT_RISE_TIME, pwm_get_risetime());
                        uart_response(OK);
                    }
                    else
